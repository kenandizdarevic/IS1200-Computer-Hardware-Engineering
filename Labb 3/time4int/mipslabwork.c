...
void user_isr( void )
{
  if(IFS(0) & 0x0080){  /* INT1IF is found at index 7, if true proceed */
    IFS(0) = IFS(0) = 0xffffff7f; /* Reset INT1 Interruption Flag */
    PORTE++;
  }
...
void labinit( void )
{
  ...
  *portE = 0;                   // turns of all the leds
  ...
  IEC(0) = IEC(0) | 0x0100;     /* Enables T2IE */
  IPC(2) = IPC(2) | 0x001C;     /* Prioritize the interrupt for Timer 2 */

  IEC(0) = IEC(0) | 0x0080;     /* Enables INT1IE */
  IPC(1) = IPC(1) | 0x1C000000;     /* Enables INT1IP, prioritize the interrupt */

  enable_interrupt();           /* Call function from assembly */
