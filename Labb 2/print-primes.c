...
int is_prime(int n){
  ...
  for(int i = 1; i <= n; i++)	// start at i = 2 because n % 1 = 0 and n % n = 0, end loop before i = n 	
  {
     if(n % i == 0)
     {
  ...
  if(counted == 2)			// if count > 0 then n is not a prime
  {
	  counted = 1;
  }	
  else		// if count == 0 then n is a prime
  {
	  counted = 0;
  }
